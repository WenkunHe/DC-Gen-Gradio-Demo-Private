...
class DCGenFluxAnyFlowPipeline(DiffusionPipeline):
    ...
    def training_rollout(
        ...
        def inference_range(lat, ts):
            ...
            for i in range(len(ts) - 1):
                ...
                # Fix: correct arg order (timestep, sample) and extract prev_sample
                lat = self.scheduler.step(noise_pred, t, lat, r).prev_sample
            return lat
    ...
    def __call__(
        ...
        for i in tqdm(range(len(timesteps) - 1), desc="Sampling"):
            ...
            # Fix: correct arg order (timestep, sample) and extract prev_sample
            latents = self.scheduler.step(noise_pred, t, latents, r).prev_sample
